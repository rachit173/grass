...
void AttentionMatrixMultiply::matmul_using_blas(Matrix_t &matrix_A, Matrix_t &matrix_B, int64_t result_idx) {
...
void AttentionMatrixMultiply::show_input_matrix() {
    Matrix_t input_matrix = this->GetInputMatrix();
    std::cout << "Input matrix: \n" << input_matrix.to_string() << std::endl;
}

void AttentionMatrixMultiply::show_result_matrix() {
    Matrix_t result_matrix = this->GetResultMatrix();
