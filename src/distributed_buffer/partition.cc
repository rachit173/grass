...
int DistributedBuffer::GetStablePartitionId() {
    int outgoing_super_partition = partition_to_be_sent_[current_round_][self_rank_];
    int stable_super_partition = machine_state_[current_round_][self_rank_].first;
    if(machine_state_[current_round_][self_rank_].first == outgoing_super_partition){
        stable_super_partition = machine_state_[current_round_][self_rank_].second;
    }
...
graph::VertexPartition* DistributedBuffer::SendPartition(int super_partition_id) {
    ...
    while(true) {
        ...
        std::cout << "Waiting on CV for some partition to be done" << std::endl;
        cv_send_.wait(lock);
    }
